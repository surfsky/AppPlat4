...
namespace App.DAL
{
    ...
    {
        UnKnown = -1,
        [UI("Mall",  "订单", typeof(OrderStatus))]      Order     = 0,
        [UI("Mall",  "维修", typeof(RepairStatus))]     Repair    = ProductType.Repair,
        [UI("Mall",  "保险", typeof(InsuranceStatus))]  Insurance = ProductType.Insurance,
        [UI("Mall",  "检测", typeof(CheckStatus))]      Check     = ProductType.Check,
        [UI("Mall",  "商品", typeof(GoodsStatus))]      Goods     = 4, //ProductType.Goods,
    ...
    public class Workflow : EntityBase<Workflow>
    {
        ...
        {
            if (type == WFType.Order)     return Order.OrderFlow;
            if (type == WFType.Repair)    return Order.RepairFlow;
            if (type == WFType.Insurance) return Order.InsuranceFlow;
            if (type == WFType.Check)     return Order.CheckFlow;
            if (type == WFType.Goods)     return Order.GoodsFlow;
            if (type == WFType.Feedback)  return Feedback.Flow;
            return Order.OrderFlow;
        }
    }
}
