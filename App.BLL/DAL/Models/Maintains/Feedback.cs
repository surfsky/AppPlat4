...
namespace App.DAL
{
    ...
    public class Feedback : EntityBase<Feedback>, IDeleteLogic
    {
        ...
        static Workflow GetFlow()
        {
            ...
            var stepDispatch = WFStep.Create(FeedbackStatus.Dispatched, Powers.FeedBackDispatch);
            var stepHandle = WFStep.Create(FeedbackStatus.Handling, Powers.FeedBackHandle);
        ...
        public Feedback ChangeStatus(FeedbackStatus status, User user, string remark = "", List<string> fileUrls = null)
        {
            ...
            this.Status = status;
            this.Save();
            this.AddHistory(user.ID, user.NickName, user.Mobile, statusName, (int)status, remark, fileUrls);
            return this;
        }
        ...
        public Feedback SetNextProcessor(User user, DateTime? dt)
        {
            ...
            if (hist != null)
            {
                hist.AssignNextUser(user?.ID, user?.NickName, user?.Mobile, dt);
            }
            return this;
        }
    }
